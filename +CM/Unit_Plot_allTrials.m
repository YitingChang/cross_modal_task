%% Plot all trial types grouped by stim types
...
%             'VariableNames', trialTypes);
        ...
        scatter(cell2mat(sorted_s5(:,2)), vvv_trailNum, 8,'filled', 'b'); 
        ...
        plot(time, spkRate_tt.TTT{1,1}(:,i), 'b',time, spkRate_tt.TTN{1,1}(:,i),'k',...
            time, spkRate_tt.VTV{1,1}(:,i), 'g',time, spkRate_tt.VTN{1,1}(:,i),'r');
        hold on
        ...
        fill(time_bw, CI_TTT, 'b', time_bw, CI_TTN, 'k',...
            time_bw, CI_VTV, 'g', time_bw, CI_VTN, 'r', 'FaceAlpha', 0.3, 'Linestyle', 'none');
        xlim([-1 1]);
        ylim([-0.5 4]);
        ...
        plot(time, spkRate_tt.VVV{1,1}(:,i),'b',time,spkRate_tt.VVN{1,1}(:,i),'k',...
            time, spkRate_tt.TVT{1,1}(:,i), 'g',time, spkRate_tt.TVN{1,1}(:,i),'r');
        hold on
        ...
        time_bw = [time, fliplr(time)];
        fill(time_bw, CI_VVV, 'b', time_bw, CI_VVN, 'k',...
            time_bw, CI_TVT, 'g', time_bw, CI_TVN, 'r','FaceAlpha', 0.3, 'Linestyle', 'none');
        xlim([-1 1]);
        ylim([-0.5 4]);
        xlabel('Time from stim onset(s)');
        ylabel('Spike rate (/25ms)');
...
%             num2str(se.userData.sessionInfo.unit_position(i)),'um\_btSig\_', num2str(btInd_sig)],'FontSize',10);
        % Save fig
        figDir = 'E:\CM_NeuralActivity_Analysis\UnitPlot_btInd_nondrifting_allTrials';
        cd(figDir);
