...
%             Xtrial = u{session,4}(:,:,:,:,initialBin,:);
            ...
            for shuffle=1:2 % 1: no shuffling; 2: shuffling
                for comp = 1:length(component_names)
                    ...
                    Mdl = fitcdiscr(Ztrial_2D(isUsedPred,:)',isClass,'discrimtype','diaglinear','CrossVal','on'); % 10-fold cross-validation of LDA
    %                 Mdl = fitcsvm(Ztrial_2D(isUsedPred,:)',isClass,'CrossVal','on');% 10-fold cross-validation of SVM
