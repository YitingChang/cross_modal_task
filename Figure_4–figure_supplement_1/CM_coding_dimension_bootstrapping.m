...
for i = 1: length(recSites)
    ...
    for p = 1:nboot
        ...
        for session=1:size(resampled_u,1)
            ...
            for q =1:2
                ...
                for b=1:B
                    ...
                    projection_on_stimulus_CD(q,session,b,1,1,1:T) = squeeze(normalized_X(:,b,1,lickDirection,:))'*CD_stim_mode; % sP_tLick 
                    projection_on_stimulus_CD(q,session,b,1,2,1:T) = squeeze(normalized_X(:,b,1,3,:))'*CD_stim_mode; % sP_tNoLick
                    projection_on_stimulus_CD(q,session,b,2,1,1:T) = squeeze(normalized_X(:,b,2,lickDirection,:))'*CD_stim_mode; % sP_vLick
                    projection_on_stimulus_CD(q,session,b,2,2,1:T) = squeeze(normalized_X(:,b,2,3,:))'*CD_stim_mode; % sP_vNoLick
                    projection_on_choice_CD(q,session,b,1,1,1:T) = squeeze(normalized_X(:,b,1,lickDirection,:))'*CD_choice_mode; % cP_tLick
                    projection_on_choice_CD(q,session,b,1,2,1:T) = squeeze(normalized_X(:,b,1,3,:))'*CD_choice_mode; % cP_tNoLick
                    projection_on_choice_CD(q,session,b,2,1,1:T) = squeeze(normalized_X(:,b,2,lickDirection,:))'*CD_choice_mode; % cP_vLick
                    projection_on_choice_CD(q,session,b,2,2,1:T) = squeeze(normalized_X(:,b,2,3,:))'*CD_choice_mode; % cP_vNoLick
                end
        ...
        resampled_projection_on_stimulus_CD(1:2,1:B,1:S,1:2,1:T,p) = squeeze(mean(projection_on_stimulus_CD,2));
        resampled_projection_on_choice_CD(1:2,1:B,1:S,1:2,1:T,p) = squeeze(mean(projection_on_choice_CD,2));
            ...
            for b=1:B
                ...
                for s=1:S
                    for d=1:2 % lick and no lick- right lick in tBlock, left in vBlock
                        ...
                        bootstrap_summary = cellfun(@(x) [mean(x), x(round(0.025*nboot)), x(round(0.975*nboot))], projection_sorted,'UniformOutput', false);
